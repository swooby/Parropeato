...
- On first launch, the app shows a diagnostics consent dialog (`maybeShowDiagnosticsConsentPrompt()` in `BaseMainActivity.kt`) asking the user to opt in or out. The preference is stored as `diagnostics_enabled` (default `false`). The user can change their choice later via Settings → Diagnostics.
